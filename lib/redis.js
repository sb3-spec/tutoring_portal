...
async function connect() {
    if (!client.isOpen()) {
        ...
        await client.open(process.env.REDIS_URL_PRODUCTION);
...
async function getCustomRange(floor, ceiling) {
    ...
    const sessions = await repository.search()
        .where('timestamp').is.greaterThanOrEqualTo(floor.getTime()/1000).and('timestamp').is.lessThanOrEqualTo(ceiling.getTime()/1000).sortBy('timestamp').all()
